...
This is a **GGUF-quantized version** of the **[BASE_REPO](https://huggingface.co/BASE_REPO)** language model — a balanced **1.7-billion-parameter** LLM designed for **efficient local inference with strong reasoning and multilingual capabilities**.
...
|----------|--------------|----------|-----------|----------------|
| Q2_K     | Very Low     | ⚡ Fastest | 880 MB   | Only on very weak devices; avoid for reasoning. |
| Q3_K_S   | Low          | ⚡ Fast    | 1.0 GB   | Minimal viability; basic completion only. |
| Q3_K_M   | Low-Medium   | ⚡ Fast    | 1.07 GB  | Acceptable for simple chat on older systems. |
| Q4_K_S   | Medium       | 🚀 Fast    | 1.24 GB  | Good balance for low-end laptops or Mac Minis. |
| Q4_K_M   | ✅ Balanced   | 🚀 Fast    | 1.28 GB  | Best overall for general use on average hardware. |
| Q5_K_S   | High         | 🐢 Medium  | 1.44 GB  | Better reasoning; slightly faster than Q5_K_M. |
| Q5_K_M   | ✅✅ High     | 🐢 Medium  | 1.47 GB  | Top pick for coding, logic, and deeper interactions. |
| Q6_K     | 🔥 Near-FP16 | 🐌 Slow    | 1.67 GB  | Excellent fidelity; great for RAG and retrieval. |
| Q8_0     | 🏆 Lossless*  | 🐌 Slow    | 2.17 GB  | Maximum accuracy; recommended when precision matters most. |
...
for QTYPE in "${QUANTS[@]}"; do
    ...
    case "$QTYPE" in
        "Q2_K")   ram="~0.9 GB"; speed="⚡ Fast"; qual="Very Low" ;;
        "Q3_K_S") ram="~1.1 GB"; speed="⚡ Fast"; qual="Low" ;;
        "Q3_K_M") ram="~1.3 GB"; speed="⚡ Fast"; qual="Low-Medium" ;;
        "Q4_K_S") ram="~1.4 GB"; speed="🚀 Fast"; qual="Medium" ;;
        "Q4_K_M") ram="~1.5 GB"; speed="🚀 Fast"; qual="Balanced" ;;
        "Q5_K_S") ram="~1.6 GB"; speed="🐢 Medium"; qual="High" ;;
        "Q5_K_M") ram="~1.7 GB"; speed="🐢 Medium"; qual="High+" ;;
        "Q6_K")   ram="~2.0 GB"; speed="🐌 Slow"; qual="Near-FP16" ;;
        "Q8_0")   ram="~2.3 GB"; speed="🐌 Slow"; qual="Lossless*" ;;
        *)        ram="~? GB"; speed="❓ Unknown"; qual="Unknown" ;;
    esac

    # Dynamic CLI example based on model capability
    if [[ "$QTYPE" =~ ^(Q5_K_M|Q6_K|Q8_0)$ ]]; then
        PROMPT="Explain how photosynthesis works in plants, including the role of chlorophyll and sunlight."
        TEMP=0.5
        MODE="thinking"
    elif [[ "$QTYPE" =~ ^(Q4_K_M|Q5_K_S)$ ]]; then
        PROMPT="Write a short limerick about a robot who loves gardening."
        TEMP=0.8
        MODE="creative"
    else
        PROMPT="Summarize what a smartphone is in one sentence."
        TEMP=0.3
        MODE="basic"
    fi
...
curl http://localhost:11434/api/generate -s -N -d '{
  "model": "hf.co/geoffmunn/Qwen3-1.7B:${QTYPE}",
